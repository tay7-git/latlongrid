...
class MgrsGridLayer (core.QgsPluginLayer):
    ...
    def showDialog(self):
        ...
        if not is_defined or int(QgsProject.instance().readEntry("SpatialRefSys", "/ProjectionsEnabled")[0]) == 0:
            if self.use_dialogs :
                QtGui.QMessageBox.information(None,'Cannot add MGRS grid', """MGRS grid alsways uses geograhical coordinates.\n\nPlease enable projection 'on the fly' CRS transformation in project settings!""",  QMessageBox.Ok)
            else :
                qgis.utils.iface.messageBar().pushMessage("Cannot add MGRS grid", """MGRS grid alsways uses geograhical coordinates.\n\nPlease enable projection 'on the fly' CRS transformation in project settings!""", QgsMessageBar.WARNING, 5)
    ...
    def setMgrsGridCrsAndExtent(self) :
        ...
        if layer == None :
            text = "Cannot get parent layer for MGRS grid !\nMGRS CRS not set!"
            QtGui.QMessageBox.information(None,'Warning', text,  QMessageBox.Ok)
            return
        ...
        # if parent layer is MGRS CRS, use it
    ...
    def generateMgrsGrid(self) :
        ...
        #progressMessageBar = self.iface.messageBar().createMessage("Generating MGRS grid...")
        ...
        if x2 - x1 > 1000 or y2 - y1 > 1000 :
            ...
            text = "Too many MGRS lines! Cannot update view!"
            if self.use_dialogs :
                QtGui.QMessageBox.information(None,'Warning', text,  QMessageBox.Ok)
            else :
